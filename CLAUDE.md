...
- **Never create a branch or push anywhere without explicit confirmation first.** A message that opens with "look at branch X" or "let's discuss/design Y" is a request to read and talk, not permission to check out a new branch, commit, or push — those are separate, explicit asks, no matter how the conversation ends up (even a fully-agreed design doc is a *draft to hand back*, not an auto-push). This holds even when a session's own setup names a default working branch: if the user has referenced an existing branch by name in conversation (e.g. "look at branch tutorial-rebuild"), that named branch is the working branch — don't create or push to a different, freshly-generated one alongside it without asking. Before any push, say which branch it's going to and wait for a yes, the same as any other hard-to-reverse action.
