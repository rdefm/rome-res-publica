...
Evidence for each Chunk C10 target, harness/test used, final `BALANCE` values, and what moved and why. Harness: `engine/campaignSim.ts`'s `simulateWar`/`simulateOneWar`, exercised via `__tests__/campaignSim.test.ts`. Economy/muster-spread targets use direct engine checks against the same file (armyEngine/battleSim), not the war harness itself — see their own rows.

**A real bug surfaced before any of this tuning could mean anything:** `campaignResolver.ts`'s Step 6 (control flips) updated `TheatreState.controllers` on a region flip but never synced that flip to the region's own cities' `CityState.owner` — and `warStanding.computeSicilyControl` reads `city.owner` directly (Sicily is C1's 4-cities-in-1-region collapse), not the coarser region-controller field. Sicily's contribution to `warScore` was therefore frozen at its starting value for the ENTIRE war, in the harness and in real play alike, no matter how the theatre map actually went. Fixed in `campaignResolver.ts`: on a flip, every `status: 'foreign'` city in that region gets `owner` set to the new controller (promoted to `'unincorporated'` on a Rome flip) — deliberately narrowed to `'foreign'`-status cities only, so an already-incorporated/unincorporated Roman city is never touched by this. Confirmed with the user before implementing (a structural finding, not a number). A second, smaller bug in `campaignSim.ts` itself (the harness's own season loop never carried `resolveCampaignSeason`'s returned `theatre`/`cities` forward — `const`, never reassigned) was found and fixed in the same pass; without it, the city-owner-sync fix above would have had no visible effect in the harness at all.

| # | Target | Evidence | Verdict |
|---|---|---|---|
| 1 | Idle Rome loses (≥70% negative outcome by 241) | `simulateWar('idle', 60, seed 1)`: **negativeOutcomeRate 1.0** (100%), medianWarLengthYears 23 (every idle war rides the 241 BC cutoff, `humbled`) | PASS |
| 2 | Competent Rome can win (≥40% positive, median 8–16y) | `simulateWar('ai', 60, seed 1)`: **positiveEndRate 1.0**, **medianWarLengthYears 9**, p90 12 (n=200 confirms: median 9, p90 11 — stable, not a seed-1 fluke) | PASS |
| 3 | Battle cadence (2–4 seasons/battle, hot war) | Same `ai` run: **medianSeasonsPerBattle 3.25** (n=200: 3.2), avgBattleCount ≈11 over a ~9y war | PASS |
| 4 | Orders time (≤60s/season, 3-army mid-war) | Not automatable — a hand-played Pace-panel measurement, no headless equivalent exists. **Flagged for manual verification**, not demonstrated here. | NOT DEMONSTRATED (manual) |
| 5 | Economy (10-cohort African army costs 25–40% of typical mid-game income) | `armyEngine.upkeepFor` on a synthetic 10-cohort `player` army in Carthage-held Africa vs. a real `cityEngine.calcCityGoldOutput`-computed baseline (4 incorporated Italian provinces, hands-off policy, + 2 mid-tier personal holdings = 80/season): **upkeep 27, ratio 33.75%** | PASS |
| 6 | Abstract/tactical parity (C8 calibration still ±10%) | `abstractResolver.test.ts`'s existing calibration suite re-run post-tuning, untouched by any C10 change — still green | PASS |
| 7 | Muster spread (picked beats emergency, equal spend) | `battleSim.simulateBattles`, picked (25 cohorts) vs. emergency (30 cohorts), 300 seeds, relationship 85, budget 300: **picked wins 65%, emergency 14.3%** (rest withdrawals) | PASS |
| 8 | M11 harness (if deferred, run now) | N/A — M11 was already built (not deferred); its own targets are recorded in `rome-military-implementation-plan.md`'s tuning log | N/A |

**Final `BALANCE` values changed this chunk** (all under `BALANCE.campaign`, all "C10 tunes" per their own doc comments):

- `resolution.controlFlipThresholdSeasons`: 2 → **12**. Needed only once the city-owner-sync bug above was fixed and territorial control started actually moving `warScore` — at 2, a single early Roman victory in Sicily locked in the whole region within 2 seasons, and combined with `armyBalance`'s own cap, wars were resolving in 1–3 years (median as low as 1 in one tuning pass). 12 seasons (3 years of uncontested holding) reads as a real, sustained pacification rather than a skirmish — not unreasonable next to the historical decade-long Lilybaeum siege it's abstracting.
- `standing.sicilyControlPerCity` / `sicilyControlLilybaeumWeight`: 10/12 → **8/10**. The original weights (max ±42 for full Sicily) let one clean regional flip alone clear the sue-tier threshold (40) almost by itself; brought down so territory is a strong signal but not a solo war-winner.
- `standing.armyBalanceMult` / `armyBalanceCap`: tried 6–9/16 mid-pass (to slow the early snowball further) but this reintroduced permanent stalemates — a weaker army-balance term meant many wars never generated enough combined signal to cross 40 at all, running to the 241 BC ceiling instead. Reverted to the original **12/20** — a strong military-dominance signal turned out to be what actually PREVENTS the harness's worst failure mode (endless stalemate), not what causes over-fast wars.
- `standing.momentumDecayMult`: tried 0.4 (faster decay, to damp battle-streak spikes) mid-pass; reverted to the original **0.6** for the same reason as above — paired with the reverted armyBalanceMult, the original value was already correct once the real bug was fixed.
- `upkeep.baseDenariiPerCohort`: 2 → **1.5**. Target #5's ratio was 74% at the original value (province income alone was too austere a comparison point) — once a defensible "typical mid-game income" baseline was computed from real production code (see target #5's evidence), 1.5 lands the ratio at a clean 33.75%, comfortably inside 25–40%.
- `muster.tiers.picked.costPerCohort`: 45 → **18**; `picked.baseVeterancy`: `'trained'` (chance-based step-up) → **`'veteran'`, guaranteed**; `muster.tiers.standard.costPerCohort`: 25 → **17**. The biggest finding of this chunk's tuning pass: at the original 3x emergency→picked cost spread, picked-tier quality could **never** be worth its price under the veterancy stat-mult system's own ceiling (raw 0.85 → veteran 1.15, only a ~35% spread) — no combination of quality-bump chances closes a 3x cohort-count gap. Verified two ways: static `armyStrength()` sums (picked ≈40% behind emergency for equal spend) and a real headless tactical battle (`battleSim`, picked losing 86% of the time). **Confirmed with the user**: rather than leave target #7 unsatisfiable, picked's price was brought down close to standard's and its quality guaranteed outright, reframing "Picked Men" as a smaller, selective muster priced for its selectivity rather than a 3x premium tier — standard's price was nudged down alongside it so the three tiers stay distinctly priced rather than colliding.

**What moved and why (narrative):** The chunk's real work wasn't picking better numbers — it was discovering that `warScore`'s territorial term was structurally inert (a wiring gap between C7's region-flip logic and C9's war-standing formula, invisible until a harness actually ran a full war end-to-end) and that the muster tiers' price/quality tables were internally inconsistent with each other (a 3x price spread against a ~35% combat-power spread can never make the expensive tier worth buying). Once those two structural issues were fixed, BALANCE tuning was fast: `controlFlipThresholdSeasons` alone swept the `ai`-policy median war length from ~1 year (too fast, region flips instantly) through a wide unstable middle (a compressed `armyBalanceMult`/`momentumDecayMult` attempt caused permanent stalemates instead) to a clean 8–16y band at threshold 12, with the OTHER standing constants ending up back at their original seeds. The muster fix required a genuine design call (picked cheaper than standard) rather than a pure number tune, and was confirmed with the user rather than assumed. Targets #1, #3, #5, #6 needed comparatively little iteration once the structural fixes landed — #1 was never in doubt (idle Rome has no armies at all), #6 was untouched by any of this chunk's changes, and #5's only lever was a single upkeep constant against a real computed income baseline. Target #4 remains the one genuinely open item — it needs a human at the Pace panel, not a headless run.
